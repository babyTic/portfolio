// =============================================================================
// PersonaHome.tsx v5 — Portfolio de Kiswendsida Cédric Dakoure
// =============================================================================
...
// ── Types ─────────────────────────────────────────────────────────────────────
// FIX 1 : fichier renommé en .tsx (syntaxe TS dans .jsx = erreur Vite)
// FIX 2 : interface et types explicites sur tous les props
...
const StarSVG = ({ color }: { color: string }) => (
  <svg viewBox="0 0 100 100" fill="none">
    <polygon
      points="50,2 61,36 97,36 68,57 79,92 50,71 21,92 32,57 3,36 39,36"
      fill={color}
      stroke={color === C.white ? C.black : C.white}
      strokeWidth="3"
    />
    <polygon
      points="50,14 57,36 80,36 62,50 69,72 50,59 31,72 38,50 20,36 43,36"
      fill={color === C.white ? C.black : C.white}
      opacity="0.22"
    />
  </svg>
)
    ...
    >
      <AnimatePresence>
        {hovered && (
          <motion.div
            style={{
              position: 'absolute',
              // FIX 3 : clé CSS dynamique → spread conditionnel (TypeScript rejette [expr]: val)
          ...
          >
            <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
              ...
              {/* FIX 3 (suite) : même correction sur chaque slash */}
              <div style={{
                position: 'absolute', top: '-8px', bottom: '-8px',
                ...(isRight ? { left: '-24px' } : { right: '-24px' }),
                width: '115%', background: C.red,
                transform: `skewX(${SKEW}deg)`, zIndex: 0,
              }} />
              <div style={{
                position: 'absolute', top: '-14px', bottom: '-14px',
                ...(isRight ? { left: '-36px' } : { right: '-36px' }),
                width: '60%', background: C.red,
                transform: `skewX(${SKEW * 0.9}deg)`, opacity: 0.85, zIndex: 0,
              }} />
              <div style={{
                position: 'absolute', top: '-18px', bottom: '-18px',
                ...(isRight ? { left: '-48px' } : { right: '-48px' }),
                width: '30%', background: C.red,
                transform: `skewX(${SKEW * 0.8}deg)`, opacity: 0.5, zIndex: 0,
              }} />
              ...
              <div style={{
                ...
                <div style={{ transform: `skewX(${-SKEW}deg)` }}>
                  <p style={{
                    fontFamily: "'Archivo Black', sans-serif", color: C.white,
                    fontSize: 'clamp(15px, 2.2vw, 21px)', letterSpacing: '0.14em',
                    lineHeight: 1, whiteSpace: 'nowrap',
                  }}>
                    {item.label}
                  </p>
                  <p style={{
                    color: C.red, fontSize: '9px', letterSpacing: '0.22em',
                    marginTop: '4px', whiteSpace: 'nowrap', fontFamily: 'monospace',
                  }}>
                    {item.jp}
                  </p>
                </div>
              ...
              {/* Pointe triangulaire */}
              {/* FIX 3 (suite) : borderLeft/borderRight dynamique */}
              <div style={{
                position: 'relative', zIndex: 1, width: 0, height: 0,
                borderTop: '26px solid transparent',
                borderBottom: '26px solid transparent',
                ...(isRight
                  ? { borderLeft: `14px solid ${C.black}` }
                  : { borderRight: `14px solid ${C.black}` }),
                flexShrink: 0,
            ...
            <motion.p
              ...
              style={{
                color: 'rgba(255,255,255,0.5)', fontSize: '8px', letterSpacing: '0.3em',
                marginTop: '5px',
                paddingLeft: isRight ? '6px' : '0',
                paddingRight: isRight ? '0' : '6px',
      ...
      >
        ...
        <div style={{
          ...
          background: hovered ? C.white : C.red,
          border: `2.5px solid ${C.white}`,
          boxShadow: `0 0 ${hovered ? 18 : 10}px ${C.red}`,
...
function POILayer({ onSelect }: { onSelect: (item: MenuSection) => void }) {
  return (
...
function MobileMenu({
  ...
  items: MenuSection[]
  onSelect: (item: MenuSection) => void
}) {
  return (
    <motion.div
      className="md:hidden"
    ...
    >
      {items.map((item, i) => (
        ...
        >
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
            <div style={{
              position: 'absolute', top: '-5px', bottom: '-5px', left: '-16px', right: '-8px',
              background: C.red, transform: `skewX(${SKEW}deg)`, zIndex: 0,
            }} />
            <div style={{
              position: 'relative', zIndex: 1, background: C.black,
              transform: `skewX(${SKEW}deg)`, padding: '0 18px', height: 42,
              display: 'flex', alignItems: 'center',
            }}>
              <div style={{ transform: `skewX(${-SKEW}deg)` }}>
                <p style={{
                  fontFamily: "'Archivo Black', sans-serif", color: C.white,
                  fontSize: 15, letterSpacing: '0.15em', lineHeight: 1, whiteSpace: 'nowrap',
                }}>
                  {item.label}
                </p>
                <p style={{ color: C.red, fontSize: 8, letterSpacing: '0.2em', marginTop: 3 }}>
                  {item.jp}
                </p>
              </div>
            </div>
...
function SectionPanel({
  ...
  // Style partagé entre motion.a et motion.p (évite la duplication)
  const sharedTextStyle = {
    fontFamily: "'Archivo Black', sans-serif",
    fontSize: 'clamp(13px, 2.5vw, 19px)' as const,
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'relative', height: 140, flexShrink: 0, background: C.red, overflow: 'hidden' }}>
        ...
        {[...Array(10)].map((_, i) => (
          <div key={i} style={{
            position: 'absolute', top: `${i * 14}%`, left: '-10%', right: '-10%',
            height: 1, background: 'rgba(0,0,0,0.12)', transform: 'rotate(-3deg)',
          }} />
        ))}
        <motion.div
          style={{ position: 'absolute', bottom: 18, left: 28, zIndex: 2 }}
        ...
        >
          <h1 style={{
            fontFamily: "'Archivo Black', sans-serif", color: C.white,
            fontSize: 'clamp(28px, 7vw, 50px)', letterSpacing: '0.2em',
            lineHeight: 1, textShadow: '3px 3px 0 rgba(0,0,0,0.4)',
          }}>
            {section.label}
          </h1>
        ...
        <div style={{
          ...
          fontFamily: "'Archivo Black', sans-serif",
          fontSize: 80, color: 'rgba(0,0,0,0.18)', lineHeight: 1,
        }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '36px 28px 16px' }}>
        {section.items.map((it, i) => {
          const isLink = typeof it === 'object' && it.url
          const label  = isLink ? it.label : it as string
          ...
          return (
            <motion.div
              ...
              style={{
                ...
                // FIX 4 : marginLeft remplace le style transform inline
                // (transform inline entre en conflit avec Framer Motion)
                marginLeft: i % 2 === 1 ? 14 : 0,
              }}
            >
              <span style={{
                color: C.red, fontFamily: "'Archivo Black', sans-serif",
                fontSize: 11, letterSpacing: '0.3em', flexShrink: 0, minWidth: 24,
              }}>
                {String(i + 1).padStart(2, '0')}
              </span>
              <motion.div
                style={{ height: 1, background: `linear-gradient(to right, ${C.red}, transparent)`, flexShrink: 0 }}
                initial={{ width: 0 }}
                animate={{ width: 36 }}
              ...
              {isLink ? (
                <motion.a
                  href={(it as { label: string; url: string }).url}
                  target="_blank"
                  ...
                  style={{
                    ...
                    textDecoration: 'none',
                    cursor: 'pointer',
                    borderBottom: `1px solid rgba(211,17,17,0.4)`,
                  }}
                  whileHover={{ color: C.white, x: 5 }}
                ...
                <motion.p
                  style={{ ...sharedTextStyle, color: C.white }}
                  whileHover={{ color: C.red, x: 5 }}
                  transition={{ duration: 0.1 }}
                >
                  {label}
                </motion.p>
        ...
        {section.id === 'contact' && (
          <motion.div
            ...
            style={{
              ...
              border: 'solid 1px rgba(211,17,17,0.3)',
          ...
          >
            {['PHP', 'Laravel', 'React', 'Node.js', 'MySQL', 'PostgreSQL', 'Python', 'Java', 'Git', 'Bootstrap'].map((tag) => (
              <span
                key={tag}
                style={{
                  background: 'rgba(211,17,17,0.15)',
                  border: 'solid 1px rgba(211,17,17,0.4)',
                  color: C.red, fontSize: 10, padding: '4px 10px',
                  fontFamily: "'Archivo Black', sans-serif", letterSpacing: '0.15em',
                  transform: `skewX(${SKEW}deg)`, display: 'inline-block',
                }}
              >
                <span style={{ display: 'inline-block', transform: `skewX(${-SKEW}deg)` }}>
                  {tag}
                </span>
              </span>
            ))}
...
function TransitionOverlay({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 50, overflow: 'hidden', pointerEvents: 'none' }}>
      <motion.div
        style={{
          position: 'absolute', width: '65%', height: '65%', top: '-5%', left: '-5%',
          background: C.black, clipPath: 'polygon(0 0, 100% 0, 75% 100%, 0 100%)',
        }}
        initial={{ x: '-105%', y: '-105%' }}
        animate={{ x: 0, y: 0 }}
      ...
      <motion.div
        style={{
          position: 'absolute', width: '65%', height: '65%', bottom: '-5%', right: '-5%',
          background: C.black, clipPath: 'polygon(25% 0, 100% 0, 100% 100%, 0 100%)',
        }}
        initial={{ x: '105%', y: '105%' }}
        animate={{ x: 0, y: 0 }}
        transition={{ duration: 0.22, ease: [0.22, 1, 0.36, 1] }}
      ...
      <motion.div
        style={{
          position: 'absolute', width: '130%', height: '28%', top: '36%', left: '-15%',
          background: C.red, clipPath: 'polygon(0 35%, 100% 0%, 100% 65%, 0 100%)',
        }}
        initial={{ x: '-120%' }}
        animate={{ x: 0 }}
      ...
      <motion.div
        style={{
          position: 'absolute', width: '160%', height: '3px', top: '49%', left: '-30%',
          background: C.white, transform: 'rotate(-8deg)',
          boxShadow: `0 0 8px ${C.white}, 0 0 24px ${C.white}`,
        }}
        initial={{ x: '-160%' }}
        animate={{ x: '160%' }}
        transition={{ duration: 0.13, delay: 0.28, ease: 'easeIn' }}
      />
      <motion.div
        style={{ position: 'absolute', inset: 0, background: C.white }}
        initial={{ opacity: 0 }}
        animate={{ opacity: [0, 0.55, 0] }}
        transition={{ duration: 0.11, delay: 0.32 }}
      />
    </div>
  )
}
...
export default function PersonaHome() {
  ...
  const pendingRef   = useRef<MenuSection | null>(null)

  // Note : police Archivo Black à déplacer dans index.html pour éviter le FOUC
  ...
  const handlePOIClick = useCallback((item: MenuSection) => {
  ...
  return (
    <div
      ...
      style={{
        ...
        overflow: 'hidden', background: C.black, cursor: 'crosshair',
      }}
    >
      {/* Fond parallaxe */}
      <motion.div
        style={{
          ...
          backgroundColor: '#1a0a0a', // fallback si l'image ne charge pas
      ...
      <AnimatePresence>
        {showMap && (
          <motion.header
            style={{ position: 'absolute', top: 28, left: 24, zIndex: 20 }}
          ...
          >
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
              ...
              <div>
                ...
                <h1 style={{
                  fontFamily: "'Archivo Black', sans-serif", color: C.white,
                  fontSize: 'clamp(22px, 4vw, 44px)', lineHeight: 0.95, letterSpacing: '0.1em',
                }}>
                  CÉDRIC
                </h1>
                <h1 style={{
                  fontFamily: "'Archivo Black', sans-serif", color: C.red,
                  fontSize: 'clamp(26px, 5vw, 54px)', lineHeight: 0.95, letterSpacing: '0.1em',
                  textShadow: 'rgba(211,17,17,0.5) 0 0 28px',
                }}>
                  DAKOURE
                </h1>
                <p style={{
                  color: 'rgba(255,255,255,0.28)', fontSize: 9, letterSpacing: '0.4em',
                  marginTop: 8, borderTop: '1px solid rgba(211,17,17,0.3)', paddingTop: 5,
                }}>
      ...
      <AnimatePresence>
        {showMap && !isTransitioning && (
          <MobileMenu items={MENU_ITEMS} onSelect={handlePOIClick} />
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {activeSection && (
          <SectionPanel key={activeSection.id} section={activeSection} onBack={handleBack} />
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {isTransitioning && (
          <TransitionOverlay key="overlay" onComplete={handleTransitionComplete} />
        )}
      </AnimatePresence>
    </div>
  )
